...
| 功能 | 当前行为 |
| --- | --- |
| 自动刷新 | 默认每 30 分钟，可在采集管理页设为 5–1440 分钟；即时任务只采当天＋前一天，先采当天；历史单号复查与滚动补采独立执行 |
...
| 历史 Pending 发现 | 只循环最近 30 天（含今天），每 30 分钟一个最多 7 天的区间；超预算拆分，过期自动任务取消 |
| 采集对账 | 发布分片自动对账，每天北京时间 03:10 全范围对账；差异报告不修改业务数据 |
| 备份恢复验证 | 全库一致性快照、附件备份、SHA-256 校验、临时库真实恢复及逐表行数核对 |

若自动刷新出现数千分片、跨天仍未采集新数据，请按 [自动采集积压修复](自动采集积压修复.md) 升级；包含保留旧任务进度的迁移和服务器构建命令。
...
历史 Pending 自动发现默认 `SAVEB_PENDING_LOOKBACK_DAYS=30`，例如 2026-09-14 只循环 2026-08-16 至 2026-09-14。旧的几年前游标自动收敛，超范围自动任务取消，已入库的数据保留。手动指定日期范围和已知历史订单复查仍独立运行。服务器更新命令及范围规则见 [采集巡检与备份说明](采集巡检与备份说明.md#最近-30-天规则的服务器更新)。
