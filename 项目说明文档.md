...
| 模式 | 入口 | 范围 |
| --- | --- | --- |
| `refresh` | Beat / `refresh_fields.py` | 默认当天＋前一天、已知未完成订单复查、一个历史滚动日 |
...
时区为 `Asia/Shanghai`。刷新间隔默认 30 分钟，现由采集管理页配置；具体调度代码和生效方式见第 11 节。outbox 每 15 秒派发，汇率每小时 05 分更新。历史滚动默认从 `2021-08-10` 开始，每轮只推进一个日分片，不扫描全部历史。
...
定时检查与设置保存锁定同一账户的 schedules 行，创建任务和推进计划在同一事务；提交失败保留原到期时间供下次检查重试。仍有自动 refresh 未完成时不堆积新的自动任务，推进下次检查计划，原任务继续执行。该机制不会取消手动或历史任务。
...
首页长任务监控已改为按分片提交进度判断停滞，并展示成功分片数、总分片数和最近进展时间。仅仅整项任务超过 35 分钟不会告警；历史未完成订单较多时，一轮自动刷新可能超过配置的调度间隔。最近成功采集仍表示整项 refresh/today 任务成功完成时间，单个分片完成不冒充整项成功。
# 采购物流查询补充
